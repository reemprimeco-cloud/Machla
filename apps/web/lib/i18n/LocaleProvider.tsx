...
import type { LocaleCode, TextDirection } from "./config";
import { directionFor, isSupportedLocale } from "./config";
...
export function LocaleProvider({
  ...
  // The server already rendered the correct <html lang dir> for
  // `initialLocale` (see app/layout.tsx), so this effect only has real
  // work to do after an in-session setLocale() call — it does not cause
  // a mismatch against the server-rendered markup on first paint.
  useEffect(() => {
  ...
  const value = useMemo<LocaleContextValue>(
    () => ({ locale, direction, messages, setLocale, t }),
    [locale, direction, messages, setLocale, t],
  );
